...
//! openstranded-convert — CLI tool for converting original Stranded II mods.
//!
...
use openstranded_convert::convert::{convert_b3d_to_glb, convert_bmp_to_png, convert_bmpf_to_fnt, convert_inf_to_ron};
use openstranded_convert::registry::register_ron;
use openstranded_convert::scanner::{build_reference_map, classify_s2s, resolve_missing_script_refs, S2sClass, S2sRefType};
use openstranded_convert::script::{convert_sectioned_file, parse_dialogue_to_lua, InfRawSource};
use openstranded_convert::util::{copy_to_staging, normalize_id, parse_inf_file, read_file_lossy, relative_path, write_ron};

#[derive(Parser)]
#[command(
    name = "openstranded-convert",
...
fn main() -> Result<()> {
    ...
    println!("openstranded-convert v{}", env!("CARGO_PKG_VERSION"));
    ...
    for script_path_ptr in &all_script_files {
        ...
        if is_plain_text && !matches!(classification, S2sClass::Dialogue { .. }) {
            ...
            let lua_content = format!(
                "-- Generated by openstranded-convert (plain text ref)\nreturn [===[\n{}]===]\n",
                content
            );
            if let Some(parent) = lua_path.parent() {
                fs::create_dir_all(parent)?;
            ...
            match classification {
                ...
                S2sClass::Msgbox { title: _ } => {
                    ...
                    let lua_content = format!(
                        "-- Generated by openstranded-convert (msgbox text)\nreturn [===[\n{}]===]\n",
                        content
                    );
                    if let Some(parent) = lua_path.parent() {
                        fs::create_dir_all(parent)?;
